#!/usr/bin/env python3
"""
Inflation adjustment utilities for IPL auction prices.

Uses India CPI data to adjust prices to constant base year values.
"""
...
def compute_inflation_factors(base_year=2024):
    ...
    ----------
    base_year : int
        Year to adjust all prices to

    Returns
    -------
    dict
        Mapping from year to adjustment factor
    """
...
def adjust_prices_for_inflation(df, base_year=2024):
    ...
    ----------
    df : pd.DataFrame
        Dataframe with 'year' and 'final_price_lakh' columns
    base_year : int
        Year to adjust all prices to
