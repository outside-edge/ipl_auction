...
def load_data():
    ...
    perf = pd.read_csv(PERF_DIR / "player_season_stats.csv")
    print(f"  Performance records: {len(perf)}")

    merged_path = JOINED_DIR / "auction_with_performance.csv"
...
def main():
    ...
    if not auction_unmatched.empty:
        ...
        auction_unmatched[["year", "player_name", "team", "final_price_lakh"]].to_csv(
            auction_unmatched_path, index=False
        )
