...
def load_data():
    """Load auction and WAR data, apply inflation adjustment."""
    auction = load_dataset(JOINED_DIR / "auction_with_performance")

    if "price_2024_cr" not in auction.columns:
        auction = adjust_prices_for_inflation(auction)
...
def format_output(df, top_n=20):
    ...
    team_col = "team_auction" if "team_auction" in df.columns else "team"
    output_cols = [
        "player_name",
        "year",
        team_col,
...
def main():
    ...
    output_path = TABS_DIR / "worst_bets.csv"
